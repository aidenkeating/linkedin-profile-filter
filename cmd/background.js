...
async function handleSearch(body) {
  ...
  const matches = criteria.filter(c => body.company.find(comp => comp.toLowerCase() === c.name.toLowerCase())
  || c.altNames.find(alt => body.company.find(comp => new RegExp(alt).test(comp.toLowerCase()))))
    .map((c) => {
...
chrome.runtime.onInstalled.addListener(async () => {
  try {
    ...
    chrome.notifications.create(null, {
      ...
      message: 'Go to https://myaccount.google.com/permissions, remove this extension and try again'
    });
  }
